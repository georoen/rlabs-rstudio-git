---
title: "RStudio  +  Git  =  🚀"
author: "jeroen.staab@posteo.de"
date: "9.3.2017"
output:
  ioslides_presentation: default
---
```{r setup, include=FALSE}
knitr::opts_chunk$set(eval = FALSE)
```



## Why R Studio?

<!-- 
As most of you I came to R because of remote sensing. But why did we all install RStudio? Many other development enviroments exist.  

An IDE houses supporting functions as *Editor*, *Console* and *Plots* integrated into one specific Program.  
Since it's relase six years ago (29.2.2011) Rstudio is housing everything we need in everydays R sessions.  
Their developers also create popular packages to expand the features of R. Includes ggplot2, R Markdown & more. 
Who of you has heared of Hadley Wickham, the Chief Scientist at RStudio?  
He latly published the book "R for Data Science", documenting these packages to a broad user base from all diverent kind of sciences.  

So RStudio is developed from Users for Users. Providing us advanced tools and user interface for developing high quality code. 
-->

- *Integrated development environment* (IDE)  
-> Editor, Console, Plots, Versioning and more.  
- *R* is a widly spread data science language.
- *RStudio* is devoloped from useRs for useRs  
- It's free and opensource ❤️  

<div class="columns-2">
[![](https://www.rstudio.com/wp-content/uploads/2016/09/RStudio-Logo-Blue-Gray-125.png)  ](https://www.rstudio.com/)  
![](https://rstudio.com/wp-content/uploads/2015/10/r-packages.png)  

<br><br>
Version 1.0.136  
**-> Check for Updates**.  
</div>



## Pane Layout (left)
<!--
Keep an eye on TR pane - *Build* (packaging) and *Git* (versioning) apeare here.
-->

<div class="columns-2">
### Editor  
- Show code in new Window  
- Jump to function (F2)  
- File Navigation  
- Multicursor (CTRL+ALT+Click)

### R Notebooks  
- Document inside code
- Save results as `.html` / `.pdf`
- Gallery view plots

```{r, echo=FALSE}
plot(cars)  #Speed and Stopping Distances of Cars
plot(sleep)  #Student's Sleep Data
plot(women)  #Average Heights and Weights for American Women
```

<br>  

### Console  
- Execute comands  
- Print output
- Comand History `CTRL+UP/DOWN`  
![GIF](https://i0.wp.com/www.rstudio.com/wp-content/uploads/2016/11/tip_console.gif?w=456)  

</div>



## Pane Layout (rigth)
<!--
History Shortcut Console only
Avoid Print() in Forloop, slows down processing
-->

<div class="columns-2">
### Data, History  & more

<br>  

### Files  
- Browse files / rename  
- `setwd()` [MORE]  
- open native filebrowser [...]  

### Packages  
- `library()` / `detach()`  
- `install.packages`  
-> install `addinlist`  

![](extras/Topright-Pane.png)

</div>



## Keyboard Shortcuts 1
### Multicursor
Select multiple lines/positions for parallel edeting  
- w. Mouse: `CRTL+ALT+Click`  
- keyboard: `CRTL+ALT+UP/DOWN`  
- rename variable:  Select it and `CRTL+ALT+K`  

### Code snippets
Text macros that are used for quickly inserting common snippets of code ([see Blog](https://support.rstudio.com/hc/en-us/articles/204463668-Code-Snippets)). 
- Try `fun`, `lib`, `apply`, ...  
- Type `fun` and press `TAB` for shifting through arguments  

<img src="https://support.rstudio.com/hc/en-us/article_attachments/201361647/snippet-completion.png"  width="80%" />  <br>

### Test out
```{r}
# type `fun` and press TAB for shifting through arguments
fun
lib
apply
```



## Keyboard Shortcuts 2
Press `SHIFT+ALT+K` for "all" shortcuts:  
![](extras/Keyboard.png)



## Cheatsheets
Open cheatsheets for `ggplot2`, `markdown`, and `more` directly from the menu bar.
![](extras/Cheatsheets.png)



## Addins
<div class="columns-2">
How to install RStudio Addins?  
```{r}
# install.packages("addinslist")
addinslist:::addinslistAddin()
```

**Example Color Picker**  
- Browse Addins  
- Install `colourpicker`  
- Click `Addins > Colour picker`  
- Returns a vector  
`c("red", "black", "green", "purple")`  
<br>
see also `colorHelper()`

![](https://i1.wp.com/deanattali.com/img/blog/colourpicker-ggmarginal-gadgets/colourpickerscrnshot.png?w=456)
</div>



## Projects

<div class="columns-2">
You already organize your files in folders.  
Why not add an extra file organizing your R sessions?  

### Project file `.Rproj`
- Automatically `setwd()`  
- Load `.Rdata` enviroment  
- Load `.Rhistory`
- Easily switch between projects
- Build packages
- Versioning with `git`  


![](extras/Projects.png)
</div>



## Why Versioninig?
<!--
While developing code files are changed - that's a fact.  
On the other hand you migth not want to touch a running system. 
How to keep track of changes?

Though this screenshot is a mockup, every file name includes date and a message
-->
Traditionell workflow:  
`edit -> save as -> `edit -> save as -> ... 
A good file name includes date and a message  

<br>

<img src="extras/VersioningFilenames.png"  width="100%" />  



## Git
Versioning tools like `git` keeps track of file changes.  
Workflow: `work` with file -> `save` -> `stage` -> `commit` ( -> `push` )  
Commit includes date and message  

<br>
<div class="columns-2">

### Organize Commits
<img src="https://git-scm.com/images/branching-illustration@2x.png" width="80%" />  

### RStudio Implementation  
<img src="extras/Git-RstudioHistory.png"  width="80%" />  
</div>



## Configure Git
<div class="columns-2">
### Install `git` on your computer
- Visit [git-scm.com](https://git-scm.com/downloads)  
<img src="extras/Git-scm_com.png"  width="70%" />  

### RStudio
- Global options  
<img src="extras/Git-RstudioGlobalOptions.png"  width="70%" />  

<br>

- Project options  
<img src="extras/Git-RstudioProjectOptions.png"  width="70%" />  

- New pane (topright)  
<img src="extras/Git-RstudioPane.png"  width="70%" />  

- Connect to cloud  
e.g. Github
</div>



## Getting Started
<div class="columns-2">

### Existing Directory
- Open directory in `terminal`. 
- Type `git init`
- `Restart` RStudio.  

<br>  

> Troubleshoot:  
Check project options.

<br>  

### New Project
`Menu > File > New Project`  
<img src="extras/Project-New.png"  width="90%" />  

</div>
## Workflow
- Stage:        Select file for next commit. Freezes state.
- Commit:       Create new node in timeline. Add a message. 
- Push & pull:  Interact with remote repository.

<img src="extras/Git-workflow.png"  width="90%" />  


  
## References
<div class="columns-2">
- [Rstudio IDE Cheatsheet](https://www.rstudio.com/wp-content/uploads/2016/01/rstudio-IDE-cheatsheet.pdf)  
- [R Notebooks](http://rmarkdown.rstudio.com/r_notebooks.html)  
- [Code Snippets](https://support.rstudio.com/hc/en-us/articles/204463668-Code-Snippets?version=1.0.136&mode=desktop)  
- [Addin](https://www.r-bloggers.com/an-awesome-rstudio-addin-for-selecting-colours-and-another-for-adding-marginal-density-plots-to-ggplot2/)  
- [Version Control in RSudio](https://support.rstudio.com/hc/en-us/articles/200532077-Version-Control-with-Git-and-SVN)  
- [Git Documentation](https://git-scm.com/doc)

<br> 

### Acess this presentation
github.com/tba...  

<br> 

<img src="extras/ThankYou.png"  width="100%" />  
</div>
